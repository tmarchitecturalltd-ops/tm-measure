...
If you just want the short version: we only collect what we need to prepare your design quote, raw camera frames stay on your device, and you can email us at any time to get your data deleted. The reference photos you choose to send with a project are uploaded to our private Google Drive folder so our architect can review them alongside your dimensions.
...
- **Our photo storage** — Google Drive, where your reference photos are stored in a private folder named after your submission ID. The folder is set to "Anyone with the link can view" so our architect can open the images, but the link is unguessable and never published.
...
TM Measure asks for two device-level permissions, and only uses them while the relevant screen is open:

- **Camera** — used for the auto-scan feature. Camera frames are processed on your device to estimate room dimensions; they are not recorded, saved, or transmitted. You can use TM Measure without granting camera access — simply type dimensions in manually instead.
...
You can revoke either permission at any time in your phone's Settings app.
